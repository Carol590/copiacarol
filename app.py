...
def calcular_sarima_homologacion(df_filt, target, steps=5):
    resultados = []
    for homologacion in df_filt['HOMOLOGACIÓN'].unique():
        pred = sarima_por_grupo(df_filt, 'HOMOLOGACIÓN', homologacion, target, steps)
        for i, mes in enumerate(['Agosto','Septiembre','Octubre','Noviembre','Diciembre']):
            resultados.append({'HOMOLOGACIÓN': homologacion, 'Mes_Nombre': mes, 'Predicción': pred[i]})
    return pd.DataFrame(resultados)

def calcular_sarima_compania(df_filt, target_col='Primas', steps=5):
    resultados = []
    for compania in df_filt['COMPAÑÍA'].unique():
        pred = sarima_por_grupo(df_filt, 'COMPAÑÍA', compania, target_col, steps)
        for i, mes in enumerate(['Agosto','Septiembre','Octubre','Noviembre','Diciembre']):
            resultados.append({'COMPAÑÍA': compania, 'Mes_Nombre': mes, 'Predicción': pred[i]})
    return pd.DataFrame(resultados)
...
df_clean = df_clean[df_clean['CIUDAD'].isin(ciudades_seleccionadas)]
...
with tab1:
    ...
    st.write(f"📍 Ciudades seleccionadas: {', '.join(ciudades_seleccionadas) if ciudades_seleccionadas else 'Todas'}")
    ...
    if 'pred_sarima' in st.session_state:
        pivot = st.session_state.pred_sarima.pivot(index='HOMOLOGACIÓN', columns='Mes_Nombre', values='Predicción').fillna(0).round(0)
        st.dataframe(pivot, use_container_width=True)
        fig = px.line(st.session_state.pred_sarima, x='Mes_Nombre', y='Predicción', color='HOMOLOGACIÓN', title="SARIMA Predicciones Homologación 2025", markers=True)
...
with tab2:
    ...
    if 'pred_auto' in st.session_state:
        pivot = st.session_state.pred_auto.pivot(index='COMPAÑÍA', columns='Mes_Nombre', values='Predicción').fillna(0).round(0)
        st.dataframe(pivot, use_container_width=True)
...
with tab3:
    ...
    if 'pred_cumpl' in st.session_state:
        pivot = st.session_state.pred_cumpl.pivot(index='COMPAÑÍA', columns='Mes_Nombre', values='Predicción').fillna(0).round(0)
        st.dataframe(pivot, use_container_width=True)
...
with tab4:
    ...
    if 'pred_gen' in st.session_state:
        pivot = st.session_state.pred_gen.pivot(index='COMPAÑÍA', columns='Mes_Nombre', values='Predicción').fillna(0).round(0)
        st.dataframe(pivot, use_container_width=True)
...
with tab5:
    ...
    if 'pred_rc' in st.session_state:
        pivot = st.session_state.pred_rc.pivot(index='COMPAÑÍA', columns='Mes_Nombre', values='Predicción').fillna(0).round(0)
        st.dataframe(pivot, use_container_width=True)
...
with tab6:
    ...
    if 'pred_soat' in st.session_state:
        pivot = st.session_state.pred_soat.pivot(index='COMPAÑÍA', columns='Mes_Nombre', values='Predicción').fillna(0).round(0)
        st.dataframe(pivot, use_container_width=True)
...
with tab7:
    ...
    if 'pred_vida' in st.session_state:
        pivot = st.session_state.pred_vida.pivot(index='COMPAÑÍA', columns='Mes_Nombre', values='Predicción').fillna(0).round(0)
        st.dataframe(pivot, use_container_width=True)
...
with tab8:
    ...
    if 'pred_nosde' in st.session_state:
        pivot = st.session_state.pred_nosde.pivot(index='COMPAÑÍA', columns='Mes_Nombre', values='Predicción').fillna(0).round(0)
        st.dataframe(pivot, use_container_width=True)
